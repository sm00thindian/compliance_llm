...
def main():
    ...
    while True:
        print(f"{Fore.YELLOW}Enter your compliance question (e.g., 'How do I assess AU-3?', 'list stigs', 'exit'):{Style.RESET_ALL}")
        query = input().strip()
        if query.lower() == 'exit':
            break
        if query.lower() == 'help':
        ...
        # Check if it's an assessment query and ask about checklist
